...
The form sends contact details, coverage including county FIPS, selected plan, billing preference, referral, and campaign notes through the existing EmailJS service/template/public-key configuration. Its fixed `to_email` template parameter is `erik@patriotsinaction.com`; EmailJS template configuration must honor that parameter. Consent and a honeypot are included. Delivery failure preserves the form and displays a retry/email fallback. A success message appears only after EmailJS acknowledges the request. Standard Preferred, Gold, Platinum, and founding Gold show their existing matching Stripe link after submission. Quote-only plans never offer unrelated checkout links. Add-ons are not included in those base subscriptions. This application does not create a server-side campaign record or confirm payment.

Artwork previews accept decodable PNG, JPG, or WebP files up to 5 MiB, stay in browser memory, and are never included in the form request. Final PNG artwork goes to the advertising contact by email: 250×250 squares and 980×300 banners. The page tells users explicitly that previewing does not submit artwork.
