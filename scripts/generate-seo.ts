...
async function candidateCatalog() {
  ...
  try {
    ...
    do {
      ...
      if (!Array.isArray(body.data)) throw new Error("invalid candidate catalog");
      remote.push(...body.data);
      if (body.nextCursor && (typeof body.nextCursor !== "string" || cursors.has(body.nextCursor) || cursors.size >= 100)) {
        throw new Error("invalid candidate pagination");
      }
      if (body.nextCursor) cursors.add(body.nextCursor);
