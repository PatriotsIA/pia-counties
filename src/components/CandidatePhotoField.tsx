...
export function CandidatePhotoField({ initialValue = "", label = "Portrait image URL", onBusy, onValueChange }: { initialValue?: string; label?: string; onBusy: (busy: boolean) => void; onValueChange?: (value: string) => void }) {
  ...
  return <div className="candidate-photo-field">
    <label className="field"><span>Upload profile picture</span>
      <input type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={async (event) => {
        ...
        try { changeUrl(await uploadCandidatePhoto(file)); }
    ...
    <label className="field"><span>{label}</span><input aria-label={label} name="image" value={url} onChange={(event) => changeUrl(event.target.value)} disabled={busy} /><small>Upload a photo above or enter a direct image URL.</small></label>
