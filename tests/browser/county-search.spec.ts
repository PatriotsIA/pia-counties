  ...
  test.describe(`${width}px county search`, () => {
    ...
    for (const path of ["/"]) {
      test(`complete scrollable results and mixed queries at ${path}`, async ({ page }) => {
        ...
        const target = results.locator('a[href="/tx/jefferson"]');
...
for (const path of ["/counties", "/tx"]) {
  test(`directory search accepts the same county and state queries at ${path}`, async ({ page }) => {
    await page.setViewportSize({ width: 320, height: 800 });
    await page.goto(path);
    const input = page.getByRole("searchbox", { name: path === "/counties" ? "Search states and counties" : "Search Texas counties" });
    const results = page.locator(".directory-grid .directory-card");
    await input.fill("texas,");
    await expect(results).toHaveCount(254);
    for (const query of ["texas, jeffson", "jefferson, texas,", "TX Jefferson"]) {
      await input.fill(query);
      await expect(results).toHaveCount(1);
      await expect(results).toHaveAttribute("href", "/tx/jefferson");
    }
    if (path === "/counties") {
      await page.getByLabel("Filter by state").selectOption("florida");
