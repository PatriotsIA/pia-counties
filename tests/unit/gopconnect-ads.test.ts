...
it("includes GOPConnect nationwide while keeping the previous county sponsors", () => {
  ...
  for (const route of ["home", "state"] as const) {
    expect(resolveAdsForSlot({ route, slot: "site-inline" })).toEqual([ad]);
  }
  ...
  for (const county of counties) {
    ...
    const resolved = resolveAdsForSlot({ ...context, limit: 7 + localCount });
    expect(resolved.filter((entry) => entry.id === GOPCONNECT_AD_ID)).toHaveLength(1);
    const baseline = resolveAdsForSlot({ ...context, limit: 6 + localCount, catalog: ads.filter((entry) => entry.id !== GOPCONNECT_AD_ID) });
    expect(resolved.filter((entry) => entry.id !== GOPCONNECT_AD_ID)).toEqual(baseline);
  }
});
